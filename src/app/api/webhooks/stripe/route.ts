...
import {
  findContactByEmail,
  updateHubspotContactPaymentFields,
} from "@/lib/hubspot";
...
async function extractFromPaymentIntent(
  ...
  const out: PaymentExtract = {
    ...
    invoiceId: expanded.invoice
      ? typeof expanded.invoice === "string"
        ? expanded.invoice
        : expanded.invoice.id
      : null,
  };
  ...
  if (
    ...
    expanded.invoice &&
    typeof expanded.invoice !== "string" &&
    expanded.invoice.metadata &&
    expanded.invoice.metadata.membership_id
  ) {
    out.membershipId = expanded.invoice.metadata.membership_id as string;
  }
...
async function updateHubspotAndEmail(ex: PaymentExtract) {
  ...
  let foundContact: Awaited<ReturnType<typeof findContactByEmail>> | null =
    null;
  ...
  if (targetContactId) {
    ...
    if (ex.paymentIntentId)
      paymentFields.stripe_payment_intent_id = ex.paymentIntentId;
    ...
    console.log(
      "[HUBSPOT] Updating payment fields for contact:",
      targetContactId
    );
